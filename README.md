...
| | |
|---|---|
| **Propose** | a claim, the reasoning behind it, a subject, and how long it has — *in 1, 2, 3, 5, 10 or 20 years*, each labelled with the year it comes to, or a particular year of your own, which wins. A span is how the thought arrives; the year is what the ledger keeps, because that is what a stranger settles it by. A hand writing its first bet gets the house rules in short above the form, once. **No account first**: a stranger writes the bet, and only the last step asks for an address — the bet waits against the key that is posted out and reaches the ledger when that key is opened, never before. The draft belongs to the address, not to the key, so asking for a second key does not write the bet twice — and a hand writing the same words again is sent to the entry it already has |
